...
static void draw_header(void)
{
    ...
    h_rule(TL, TR);
    blank_row();
    centre_row(BOLD FG_YELLOW, "\xe2\x82\xb3  BUDGET TRACKER");
...
static void draw_summary(void)
{
    ...
    blank_row();
    snprintf(buf, sizeof(buf), "INCOME    +  \xe2\x82\xb3 %.2f", currentincome);
    centre_row(FG_GREEN BOLD, buf);
    snprintf(buf, sizeof(buf), "EXPENSE   -  \xe2\x82\xb3 %.2f", currentexpense);
    centre_row(FG_RED BOLD, buf);

    printf(FG_CYAN VL FG_GRAY); rep("\xe2\x94\x80", BOX_W); printf(FG_CYAN VL "\n" RESET);

    snprintf(buf, sizeof(buf), "BALANCE      \xe2\x82\xb3 %.2f", bal);
...
static void draw_menu(void)
{
    ...
    /* badge + label: pre-rendered with escapes */
    ...
    for (int i = 0; i < 5; i++) {
        int vlen = (int)strlen(items[i].label); /* visible length */
        printf(FG_CYAN VL RESET "  %s%s%s" RESET "%*s" FG_CYAN VL "\n" RESET,
    ...
    blank_row();
    h_rule(BL, BR);
    printf("\n  " BOLD FG_CYAN "\xc2\xbb\xc2\xbb " RESET);
}
...
/* ── linked-list & file helpers (unchanged logic) ─────────── */
...
static Node *list_read(const char *fn, Node **tout)
{
    ...
    fseek(fp,0,SEEK_END);
    int entries=(int)(ftell(fp)/sizeof(Node)); rewind(fp);
    Node *h=NULL,*t=NULL;
    for(int i=0;i<entries;i++){
        Node *n=malloc(sizeof(Node)); if(!n){ fclose(fp); exit(1); }
                          ...
                          double *total, const char *fn)
{
    ...
    printf(FG_CYAN "  " VL RESET "\n");
    printf(FG_CYAN "  " VL RESET "  " BOLD "Amount " DIM "(\xe2\x82\xb3)" RESET ":  ");
...
static void screen_records(Node *head, int is_income)
{
    ...
    if(!head){
        ...
        printf(FG_CYAN "  " VL FG_GRAY); rep("\xe2\x94\x80", BOX_W-4); printf(FG_CYAN VL "\n" RESET);
        ...
        for(Node *p=head; p; p=p->next){
            ...
            printf(FG_CYAN "  " VL RESET
                   "  "FG_GRAY"%-18s"RESET"  %s%s\xe2\x82\xb3 %-10.2f"RESET"  "FG_YELLOW"%-12s"RESET"  "FG_CYAN VL"\n"RESET,
        ...
        char buf[48];
        snprintf(buf,sizeof(buf),"\xe2\x82\xb3 %.2f  (%d entries)",total,count);
        printf(FG_CYAN "  " VL RESET
...
int main(void)
{
    ...
    do {
        ...
        draw_summary();
        draw_menu();
        if(scanf("%d",&opt)!=1){ opt=0; continue; }
        switch(opt){
